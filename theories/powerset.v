...
Lemma ap_partial_union : forall (A B : Type) (U T : M Pe B) 
  (f g : A -> (M Pe) B) {pf},
  ap (partial_union B) U T pf = Union B U T.
Proof.
  intros. unfold partial_union.  unfold ap. simpl. reflexivity.
Qed.
...
Proof.
  intros. exists I. simpl.
  symmetry. apply Extensionality_Ensembles. unfold Same_set. unfold Included.
  split ; intros.
  + unfold In in *. inversion H. destruct H0. 
    rewrite ap_partial_union with (A := A) in H1. destruct H1. 
    * apply Union_introl. unfold apply_f. exists x1. split ; easy.
    * apply Union_intror. unfold apply_f. exists x1. split ; easy.
    * apply f. 
    * apply g.
  + inversion H ; inversion H0 ; destruct H2 ; unfold apply_f ; exists x2 ; 
    split ; try apply H2 ; rewrite ap_partial_union with (A := A) ; 
    try apply f ; try apply g.
    * apply Union_introl. apply H3.
    * apply Union_intror. apply H3.
Qed.
